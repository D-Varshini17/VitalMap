...
class HealthUiAdapter {
  ...
  static String missingText(Map<String, dynamic> item) {
    final missing = (item['missing_inputs'] as List<dynamic>? ?? [])
        ...
        .toList();
    if (missing.isEmpty) return 'More values needed';
    if (missing.length == 1) return '${missing.first} needed';
    return '${missing.length} values needed';
  }

  static String missingActionText(Map<String, dynamic> item) {
    final indexName = item['index_name']?.toString() ?? 'this indicator';
    final missing = (item['missing_inputs'] as List<dynamic>? ?? [])
        .map((value) => cleanKey(value.toString()))
        .where((value) => value.isNotEmpty)
        .toList();
    if (missing.isEmpty) return 'Add more values to improve this insight';
    return 'Add ${_joinReadable(missing)} to calculate $indexName';
  }

  static String missingSummaryForOrgan(
    String organ,
    List<Map<String, dynamic>> items,
  ) {
    if (items.isEmpty) return 'Add values to calculate.';
    final normalized = organ.trim().toLowerCase();
    if (normalized == 'kidney') return 'Add creatinine to calculate eGFR';
    if (normalized == 'lung' || normalized == 'lungs') {
      return 'Add SpO2 to check oxygen saturation';
    }
    if (normalized == 'liver') {
      return 'Add AST, ALT, and platelets for liver indicators';
    }
    if (normalized == 'heart') {
      return 'Add triglycerides and HDL to calculate AIP';
    }
    if (normalized.contains('metabolic') || normalized == 'diabetes') {
      return 'Add triglycerides and fasting glucose to calculate TyG';
    }
    if (normalized == 'inflammation') {
      return 'Add neutrophils and lymphocytes to calculate NLR';
    }
    if (normalized == 'pancreas') {
      return 'Add lipase and amylase to calculate LAR';
    }
    if (normalized == 'cancer awareness') {
      return 'Add available awareness markers to review this section';
    }
    final names = <String>{};
    for (final item in items) {
      for (final raw in item['missing_inputs'] as List<dynamic>? ?? []) {
        final label = cleanKey(raw.toString());
        if (label.isNotEmpty) names.add(label);
      }
    }
    if (names.isEmpty) return '${items.length} values needed';
    if (names.length == 1) return 'Add ${names.first}';
    return '${names.length} values needed';
  }

  static String _joinReadable(List<String> values) {
  ...
  static Map<String, dynamic> _approvedValuesUsed(
    String indexName,
    Map<String, dynamic> values,
  ) {
    final allowed = switch (indexName) {
      'AIP' => {'triglycerides_mg/dL', 'hdl_mg/dL'},
      'TyG' => {'triglycerides_mg/dL', 'fasting_glucose_mg/dL'},
      'APRI' => {'ast', 'platelets'},
      'FIB-4' => {'age', 'ast', 'alt', 'platelets'},
      'FLI' => {'bmi', 'waist_cm', 'ggt', 'triglycerides_mg/dL'},
      'NAFLD Fibrosis Score' => {
          'age',
          'bmi',
          'ast',
          'alt',
          'platelets',
          'glucose_pattern_flag',
          'albumin',
        },
      'NLR' => {'neutrophils', 'lymphocytes'},
      'eGFR' => {'age', 'sex', 'creatinine_mg/dL'},
      'SpO2' || 'SpO₂' => {'spo2_%'},
      'LAR' => {'lipase', 'amylase'},
      'AFP' => {'afp'},
      'CA 15-3' => {'ca15_3'},
