...
Set app logo:
- The repository already includes the app logo at `frontend/assets/logo_medid.jpeg` and launcher artwork in `frontend/assets/images/logo/`. These paths are declared in `frontend/pubspec.yaml` and used by the app; no additional logo file is required.
...
Notes:
- The `flutter_launcher_icons` package will generate Android and iOS launcher icons and adaptive icons using `assets/images/logo/app_icon_padded.png` and `assets/images/logo/app_icon_foreground.png` as configured in `frontend/pubspec.yaml`.
...
- Verify `https://<your-render-service>/health` returns `{"status":"ok"}`. If it returns 404 or `/docs` shows an older API contract, manually redeploy from the latest GitHub `main` branch and confirm Render uses `backend/Dockerfile` with the repository root as its Docker context.
